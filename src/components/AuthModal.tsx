...
export const AuthModal: React.FC = () => {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault(); setError(''); try { if(mode==='login') await login(email,password); else await signup(name,email,password,role,adminCode); } catch (err:any) { setError(err.message || 'Autentikasi gagal.'); }};
  return <div className="fixed inset-0 z-[70] grid place-items-center p-4 bg-black/50 backdrop-blur-sm"><form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl border border-pink-100">
    ...
    <label className="text-xs font-semibold">Kata sandi</label><input required minLength={8} type="password" value={password} onChange={e=>setPassword(e.target.value)} className="auth-input" />
